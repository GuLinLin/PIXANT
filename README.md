...
> PIXANT.imp <-  PIXANT(demoData, MaxIterations = 20, MaxIterations0 = 20, num.trees = 100, mtry = floor(sqrt(ncol(demoData))),
>                       initialLinearEffects = 0, ErrorTolerance = 0.001, indPhen = 7, missingSize = 500, seed = 123,
>                       randomseed = FALSE, replace = TRUE, decreasing = TRUE, verbose = TRUE, sampsize = NULL,
>                       max.depth = NULL, xtrue = NA)
```
...
List of 5
 $ ximp    :'data.frame':	10000 obs. of  8 variables:
  ...
  ..$ Phenotype7: num [1:10000] -0.766 -0.534 0.994 -0.641 -0.583 ...
...
> library("PIXANT")
> phen <- data.table("phenotype.txt", header=T)
> SC <- sampleScore(phen, use="pairwise", method="pearson", adjust="fdr", alpha=.05)
```
